...
```
  ...
  *  `archetypeArtifactId` and `archetypeGroupId` are used to define the example
    project structure.
  *  `groupId` is your organisation's Java package name prefix; for example,
    `com.mycompany`.
...
```
  ...
  *  `stagingLocation` is the storage bucket that Cloud Dataflow will use for the
...
If you wish to clean up the Maven project that you generated, run `cd .. && rm -R
